...
_latest_state = {
    ...
    "vibration_mm_s": 0.5, "rpm": 0.0, "power_kw": 0.0,
    "status": "STOPPED", "faults": "NONE", "severity": "HEALTHY", "timestamp": "",
}
_sensor_history = []
...
_motor_state = {
    ...
    "shutdown_reason":    "",
    "start_time":         None,    # for ramp-up simulation
    "stop_time":          None,    # for ramp-down simulation
    "stop_type":          None,    # "normal" or "emergency"
    "temperature_at_stop": 25.0,
}
...
.header { background:linear-gradient(135deg,#ffffff,#f0f6ff,#e8f0ff); padding:12px 28px; display:flex; align-items:center; justify-content:space-between; box-shadow:0 2px 15px rgba(37,99,235,0.12); border-bottom:2px solid #bfdbfe; height:60px; }
...
.live-badge { background:rgba(34,197,94,0.1); border:1px solid #22c55e; color:#16a34a; padding:5px 14px; border-radius:20px; font-size:0.75rem; font-weight:700; display:flex; align-items:center; gap:8px; }
...
input[type=range]::-webkit-slider-thumb { appearance:none; width:16px; height:16px; background:linear-gradient(135deg,#2563eb,#0ea5e9); border-radius:50%; cursor:pointer; }
...
.fault-btn.active { background:#fff0f0; border-color:#ef4444; color:#ef4444; }
...
.run-btn:disabled { background:#94a3b8; box-shadow:none; cursor:not-allowed; }
.auto-badge { text-align:center; margin-top:8px; color:#94a3b8; font-size:0.7rem; }
.countdown { color:#2563eb; font-weight:700; }
/* MOTOR CONTROL */
.motor-ctrl-section { margin-top:4px; }
.motor-status-badge { text-align:center; padding:6px; font-size:0.75rem; font-weight:700; border-radius:8px; margin-bottom:10px; letter-spacing:0.5px; }
.msb-stopped  { background:#f1f5f9; color:#64748b; border:1px solid #cbd5e1; }
.msb-starting { background:#fefce8; color:#854d0e; border:1px solid #fde047; animation:badge-pulse 0.8s infinite; }
.msb-running  { background:#f0fdf4; color:#15803d; border:1px solid #86efac; }
.msb-stopping { background:#fff7ed; color:#9a3412; border:1px solid #fed7aa; }
.msb-shutdown { background:#fff1f2; color:#be123c; border:1px solid #fda4af; animation:badge-pulse 1s infinite; }
@keyframes badge-pulse { 0%,100%{opacity:1;} 50%{opacity:0.6;} }
.motor-ctrl-btns { display:grid; grid-template-columns:1fr 1fr; gap:7px; margin-bottom:8px; }
.ctrl-btn { padding:9px 6px; border:none; border-radius:9px; font-size:0.78rem; font-weight:700; cursor:pointer; transition:all 0.2s; }
.ctrl-btn.start { background:linear-gradient(135deg,#16a34a,#22c55e); color:#fff; }
.ctrl-btn.stop  { background:linear-gradient(135deg,#64748b,#94a3b8); color:#fff; }
.ctrl-btn:disabled { opacity:0.4; cursor:not-allowed; }
.shutdown-btn { width:100%; padding:10px; border:none; border-radius:9px; font-size:0.82rem; font-weight:800; cursor:pointer; background:linear-gradient(135deg,#dc2626,#ef4444); color:#fff; box-shadow:0 4px 14px rgba(220,38,38,0.35); letter-spacing:0.5px; }
...
/* UIPATH LOG */
.uipath-log-box { background:#f8faff; border:1px solid #bfdbfe; border-radius:10px; padding:10px; margin-top:10px; }
.uipath-log-title { font-size:0.65rem; font-weight:700; color:#2563eb; letter-spacing:2px; text-transform:uppercase; margin-bottom:7px; }
.uipath-log-list { max-height:120px; overflow-y:auto; display:flex; flex-direction:column; gap:4px; }
.uientry { padding:5px 9px; border-radius:6px; font-size:0.71rem; border-left:3px solid; }
.uientry.shutdown { background:#fff1f2; border-color:#ef4444; color:#991b1b; }
.uientry.warning  { background:#fffbeb; border-color:#f59e0b; color:#92400e; }
.uientry.approved { background:#f0fdf4; border-color:#22c55e; color:#15803d; }
.uientry.info     { background:#eff6ff; border-color:#2563eb; color:#1e40af; }
.uientry .uet { font-size:0.61rem; color:#94a3b8; }
.uientry .uem { font-weight:600; margin-top:1px; }
/* TREND */
...
.status-card.stopped  { background:linear-gradient(135deg,#f8fafc,#f1f5f9); border:1px solid #cbd5e1; }
...
.stopped .status-dot  { background:#94a3b8; animation:none; }
...
.stopped .status-info h3  { color:#64748b; }
...
.metric-value { font-size:1.7rem; font-weight:800; color:#1e3a5f; font-family:monospace; transition:color 0.5s; }
...
.metric-trend.up { color:#ef4444; } .metric-trend.down { color:#2563eb; } .metric-trend.stable { color:#94a3b8; }
.fault-tags { display:flex; flex-wrap:wrap; gap:5px; margin-bottom:12px; }
.fault-tag { background:#fff0f0; border:1px solid #fca5a5; color:#dc2626; padding:3px 10px; border-radius:20px; font-size:0.7rem; font-weight:600; }
...
.history-alert.show { display:block; }
...
.status-label{color:#2563eb;} .obs-label{color:#7c3aed;} .root-label{color:#0891b2;} .action-label{color:#d97706;} .risk-label{color:#dc2626;}
...
.msg { padding:10px 13px; border-radius:12px; font-size:0.8rem; line-height:1.6; max-width:96%; }
.msg.user { background:linear-gradient(135deg,#2563eb,#0ea5e9); color:#fff; align-self:flex-end; border-radius:12px 12px 4px 12px; }
.msg.agent { background:#f8fafc; border:1px solid #e2e8f0; align-self:flex-start; border-radius:4px 12px 12px 12px; }
.msg .role { font-size:0.68rem; font-weight:700; opacity:0.7; margin-bottom:4px; }
.msg-section { font-size:0.72rem; font-weight:700; color:#2563eb; margin:6px 0 2px; text-transform:uppercase; }
.msg-line { color:#475569; margin:2px 0; }
.msg-bullet { color:#475569; margin:2px 0; padding-left:12px; }
.suggestions { padding:8px 14px; border-top:1px solid #f1f5f9; display:flex; flex-wrap:wrap; gap:6px; flex-shrink:0; }
.suggestion { background:#eff6ff; border:1px solid #bfdbfe; color:#2563eb; padding:4px 12px; border-radius:16px; font-size:0.72rem; cursor:pointer; transition:all 0.2s; }
.suggestion:hover { background:#2563eb; color:#fff; }
.spinner { display:none; padding:8px 14px; color:#94a3b8; font-size:0.78rem; align-items:center; gap:8px; flex-shrink:0; }
.typing span { display:inline-block; width:6px; height:6px; background:#94a3b8; border-radius:50%; animation:typing 1s infinite; }
.typing span:nth-child(2){animation-delay:0.2s;} .typing span:nth-child(3){animation-delay:0.4s;}
@keyframes typing { 0%,60%,100%{transform:translateY(0);} 30%{transform:translateY(-6px);} }
.chat-input-row { display:flex; gap:8px; padding:12px 14px; border-top:1px solid #e2e8f0; flex-shrink:0; }
.chat-input { flex:1; border:1px solid #e2e8f0; border-radius:10px; padding:10px 14px; font-size:0.83rem; outline:none; }
.chat-input:focus { border-color:#2563eb; }
.send-btn { background:linear-gradient(135deg,#2563eb,#0ea5e9); color:#fff; border:none; border-radius:10px; padding:10px 18px; font-weight:700; cursor:pointer; font-size:0.83rem; }
</style>
</head>
<body>
<div class="header">
  <div class="header-brand">
    LOGO_PLACEHOLDER
    <div class="header-left"><h1>Motor<span>Mind</span> AI</h1><p>3-Phase 400V · Real-Time Predictive Maintenance · UiPath Maestro BPMN</p></div>
  </div>
...
<div class="main">
  ...
  <div class="sidebar">
    ...
    <!-- MOTOR CONTROL -->
    <div class="motor-ctrl-section">
      <div class="sidebar-title">🔌 Motor Control</div>
      <div class="motor-status-badge msb-stopped" id="motor-badge">⏹ MOTOR STOPPED</div>
      <div class="motor-ctrl-btns">
        <button class="ctrl-btn start" id="btn-start" onclick="motorControl('start')">▶ Start</button>
        <button class="ctrl-btn stop"  id="btn-stop"  onclick="motorControl('stop')" disabled>⏹ Stop</button>
      </div>
      <button class="shutdown-btn" id="btn-emergency" onclick="motorControl('shutdown')" disabled>🚨 URGENT SHUTDOWN</button>
      <button class="shutdown-btn reset-btn" id="btn-reset" onclick="motorControl('reset')" disabled style="margin-top: 10px;">🔄 Reset System</button>
    </div>

    <!-- UIPATH MESSAGES -->
    <div class="uipath-log-box" style="margin-top:10px;">
      <div class="uipath-log-title">🔗 UiPath Messages</div>
      <div class="uipath-log-list" id="uipath-log-list">
        <div style="font-size:0.72rem;color:#94a3b8;text-align:center;padding:8px">No messages yet</div>
      </div>
    </div>
    <div class="divider"></div>
    ...
    <div class="trend-section">
      ...
      <div class="trend-item"><div class="trend-name">Current</div><div class="trend-bar-bg"><div class="trend-bar" id="tr-curr" style="width:0%;background:#22c55e"></div></div><div class="trend-arrow" id="ta-curr">—</div></div>
      <div class="trend-item"><div class="trend-name">Temp °C</div><div class="trend-bar-bg"><div class="trend-bar" id="tr-temp" style="width:23%;background:#22c55e"></div></div><div class="trend-arrow" id="ta-temp">—</div></div>
      <div class="trend-item"><div class="trend-name">Vibration</div><div class="trend-bar-bg"><div class="trend-bar" id="tr-vib" style="width:0%;background:#22c55e"></div></div><div class="trend-arrow" id="ta-vib">—</div></div>
      <div class="trend-item"><div class="trend-name">RPM</div><div class="trend-bar-bg"><div class="trend-bar" id="tr-rpm" style="width:0%;background:#22c55e"></div></div><div class="trend-arrow" id="ta-rpm">—</div></div>
    </div>
    <div class="divider"></div>
    <div id="sidebar-status" style="font-size:0.72rem;color:#64748b;text-align:center;">⏹ Motor Stopped</div>
  </div>
  ...
  <div class="center">
    ...
    <div class="status-card stopped" id="status-card">
      ...
      <div class="status-info"><h3 id="status-title">⏹ Motor Stopped</h3><p id="status-sub">Press Start to begin monitoring</p></div>
      <div class="status-right" id="status-time"></div>
    </div>
    <div class="metrics">
      <div class="metric-card" id="mc-rpm"><div class="metric-label">RPM</div><div class="metric-value" id="m-rpm">0</div><div class="metric-trend stable" id="mt-rpm">—</div></div>
      <div class="metric-card" id="mc-temp"><div class="metric-label">Temperature</div><div class="metric-value" id="m-temp">25</div><div class="metric-unit">°C · limit 80</div><div class="metric-trend stable" id="mt-temp">—</div></div>
      <div class="metric-card" id="mc-vib"><div class="metric-label">Vibration</div><div class="metric-value" id="m-vib">0.0</div><div class="metric-unit">mm/s · limit 4.5</div><div class="metric-trend stable" id="mt-vib">—</div></div>
      <div class="metric-card" id="mc-curr"><div class="metric-label">Current</div><div class="metric-value" id="m-curr">0.0</div><div class="metric-unit">A · rated 15.2</div><div class="metric-trend stable" id="mt-curr">—</div></div>
      <div class="metric-card" id="mc-volt"><div class="metric-label">Voltage</div><div class="metric-value" id="m-volt">400</div><div class="metric-unit">V · rated 400</div><div class="metric-trend stable" id="mt-volt">—</div></div>
      <div class="metric-card" id="mc-pow"><div class="metric-label">Power</div><div class="metric-value" id="m-pow">0.0</div><div class="metric-unit">kW · rated 7.5</div><div class="metric-trend stable" id="mt-pow">—</div></div>
    </div>
    <div id="fault-container"></div>
    <div class="diag-card">
      <div class="diag-header">🤖 AI DIAGNOSIS</div>
      <div id="diagnosis-content"><div class="placeholder">Start motor and run diagnosis to see AI analysis</div></div>
    </div>
  ...
  <div class="chat-panel">
    ...
    <div class="chat-messages" id="chat-messages">
      <div class="msg agent"><div class="role">MotorMind Agent</div>Hello! I monitor your motor in real time. Start the motor using the control panel, then run a diagnosis or ask me anything.</div>
    </div>
...
let motorRunning    = false;   // local mirror of server state
let startTime       = null;    // for ramp-up animation
let stopTime        = null;
let stopType        = null;    // 'normal' | 'emergency'
let tempAtStop      = 25;
...
async function motorControl(action) {
  ...
  } catch(e) { console.error('motorControl error', e); }
}

function applyMotorState(data) {
  motorRunning = data.running || data.ramping_up || false;
  startTime    = data.start_time ? (Date.now() - data.elapsed_ms) : null;
  stopTime     = data.stop_time  ? (Date.now() - data.stop_elapsed_ms) : null;
  stopType     = data.stop_type  || null;
  tempAtStop   = data.temp_at_stop || 25;
  ...
  if (btnReset) btnReset.style.display = data.shutdown_requested ? 'block' : 'none';

  if (data.shutdown_requested) {
    badge.className='motor-status-badge msb-shutdown'; badge.textContent='🚨 EMERGENCY SHUTDOWN';
    btnStart.disabled=true; btnStop.disabled=true; btnEmerg.disabled=true; btnReset.disabled=false;
    card.className='status-card critical';
    title.textContent='🚨 Emergency Shutdown'; sub.textContent='All systems halted — press RESET to clear';
    sideEl.textContent='🚨 Emergency Shutdown';
    // Zero out all metrics immediately
    animateValue('m-rpm','0'); animateValue('m-curr','0.0'); animateValue('m-pow','0.0');
    animateValue('m-volt','0'); animateValue('m-vib','0.0');
  } else if (data.ramping_up) {
    badge.className='motor-status-badge msb-starting'; badge.textContent='⟳ STARTING...';
    btnStart.disabled=true; btnStop.disabled=true; btnEmerg.disabled=false;
    card.className='status-card healthy';
    title.textContent='⟳ Motor Starting...'; sub.textContent='Ramping up — please wait 4 seconds';
    sideEl.textContent='⟳ Starting...';
    ...
    card.className='status-card healthy';
    title.textContent='✓ Motor Running'; sub.textContent='All parameters nominal';
    ...
    title.textContent='⟳ Motor Stopping...'; sub.textContent='Cooling down...';
    sideEl.textContent='⟳ Stopping...';
  } else {
    badge.className='motor-status-badge msb-stopped'; badge.textContent='⏹ MOTOR STOPPED';
    btnStart.disabled=false; btnStop.disabled=true; btnEmerg.disabled=true;
    card.className='status-card stopped';
    title.textContent='⏹ Motor Stopped'; sub.textContent='Press Start to begin monitoring';
    sideEl.textContent='⏹ Motor Stopped';
    animateValue('m-rpm','0'); animateValue('m-curr','0.0'); animateValue('m-pow','0.0');
  }
}
...
function animateValue(id, val) {
  ...
  const nv=String(val);
  if(el.textContent===nv) return;
  el.style.transform='scale(1.08)'; el.textContent=nv;
...
function updateTrendBars() {
  ...
  if(vta){vta.textContent=vt.arrow;}
  const defs=[
    {key:'current_a',max:25,warn:68,el:'tr-curr',ta:'ta-curr'},
    {key:'temperature_c',max:110,warn:73,el:'tr-temp',ta:'ta-temp'},
    {key:'vibration_mm_s',max:12,warn:38,el:'tr-vib',ta:'ta-vib'},
    {key:'rpm',max:1500,warn:15,el:'tr-rpm',ta:'ta-rpm',invert:true},
  ];
  defs.forEach(f=>{
    const vals=history.map(r=>parseFloat(r[f.key]||0));
    const latest=vals[vals.length-1];
    const pct=Math.min(100,(latest/f.max)*100);
    let color;
    if(f.invert) color=latest<100?'#ef4444':latest<500?'#f59e0b':'#22c55e';
    else color=pct>f.warn*1.2?'#ef4444':pct>f.warn?'#f59e0b':'#22c55e';
    const bar=document.getElementById(f.el);
    if(bar){bar.style.width=pct+'%';bar.style.background=color;}
    const t=trendArrow(vals);
    const ta=document.getElementById(f.ta);
    if(ta) ta.textContent=t.arrow;
  });
}
...
function checkHistoryAnomalies() {
  ...
  if(rpmDrop>150) alerts.push(`RPM dropped ${rpmDrop.toFixed(0)} in last 5s`);
  const currRise=currs[currs.length-1]-currs[0];
  if(currRise>3) alerts.push(`Current rising +${currRise.toFixed(1)}A`);
  if(temps.length>=10){const tr=temps[temps.length-1]-temps[0];if(tr>8)alerts.push(`Temperature rising +${tr.toFixed(1)}°C`);}
...
// ─── SENSOR FETCH (every 1s) ─────────────────────────────────────────────────
async function fetchLiveSensors() {
  // Poll server state every tick
  try {
    const stateResp = await fetch('/api/motor/state');
    const state = await stateResp.json();
    applyMotorState(state);

    const isRunning   = state.running   || state.ramping_up;
    const isRampDown  = state.ramping_down;
    const isShutdown  = state.shutdown_requested;
    const elapsed     = state.elapsed_ms || 0;
    const stopElapsed = state.stop_elapsed_ms || 0;
    const tAtStop     = state.temp_at_stop || 25;

    if (isShutdown) {
      // Emergency stop — all values instantly 0 except temperature cooling
      const tempNow = Math.max(25, tAtStop - (stopElapsed / 1000) * 0.3);
      animateValue('m-rpm','0'); animateValue('m-curr','0.0');
      animateValue('m-pow','0.0'); animateValue('m-volt','0');
      animateValue('m-vib','0.0');
      animateValue('m-temp', tempNow.toFixed(1));
      return;
    }

    if (isRampDown) {
      // Normal stop — RPM and current decay over 3s, temperature cools slowly
      const pct = Math.max(0, 1 - stopElapsed / 3000);
      const payload = {
        load_pct: parseFloat(document.getElementById('load').value||70) * pct,
        voltage_v: parseFloat(document.getElementById('voltage').value||400),
        rpm_setpoint: parseFloat(document.getElementById('rpm').value||1450) * pct,
        fault_bearing:false, fault_overtemp:false, fault_overcurrent:false, fault_voltage_drop:false,
      };
      const resp = await fetch('/api/sensors',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      const s = await resp.json();
      // Override with physics: decay RPM/current, keep voltage, cool temperature
      const rpmNow  = (parseFloat(s.rpm||0) * pct).toFixed(1);
      const currNow = (parseFloat(s.current_a||0) * pct).toFixed(2);
      const powNow  = (parseFloat(s.power_kw||0) * pct).toFixed(3);
      const tempNow = Math.max(25, tAtStop - (stopElapsed / 1000) * 0.5).toFixed(1);
      animateValue('m-rpm', rpmNow);
      animateValue('m-curr', currNow);
      animateValue('m-pow', powNow);
      animateValue('m-temp', tempNow);
      animateValue('m-volt', s.voltage_v);
      animateValue('m-vib', (parseFloat(s.vibration_mm_s||0) * pct).toFixed(3));
      return;
    }

    if (!isRunning) return; // Motor fully stopped, nothing to update

    const payload = {
      load_pct: parseFloat(document.getElementById('load').value||70),
      voltage_v: parseFloat(document.getElementById('voltage').value||400),
      rpm_setpoint: parseFloat(document.getElementById('rpm').value||1450),
    ...
    if (state.ramping_up) {
      // Ramp-up: values start at 0 and reach nominal over 4 seconds
      const pct = Math.min(1, elapsed / 4000);
      const rpmNow  = (parseFloat(s.rpm||0) * pct).toFixed(1);
      const currNow = (parseFloat(s.current_a||0) * pct).toFixed(2);
      const powNow  = (parseFloat(s.power_kw||0) * pct).toFixed(3);
      const tempNow = Math.max(25, 25 + (parseFloat(s.temperature_c||25) - 25) * pct).toFixed(1);
      const vibNow  = (parseFloat(s.vibration_mm_s||0) * pct).toFixed(3);
      animateValue('m-rpm', rpmNow);
      animateValue('m-curr', currNow);
      animateValue('m-pow', powNow);
      animateValue('m-temp', tempNow);
      animateValue('m-volt', s.voltage_v);
      animateValue('m-vib', vibNow);
    } else {
      // Normal running — show live values
      ...
      updateTrendBars(); checkHistoryAnomalies(); updateMetrics(s, s.status||'HEALTHY');
      ...
      if (card) {
        ...
        document.getElementById('status-sub').textContent = liveStatus.replace(/_/g,' ');
...
function computeLiveStatus(s) {
  const v=parseFloat(s.voltage_v||400),i=parseFloat(s.current_a||0),
        t=parseFloat(s.temperature_c||25),vib=parseFloat(s.vibration_mm_s||0),rpm=parseFloat(s.rpm||0);
  if(i>22)            return 'CRITICAL_OVERCURRENT';
  ...
  if(rpm<50&&motorRunning) return 'IDLE';
  return 'HEALTHY';
}
...
async function runDiagnosis(silent=false) {
  ...
  // Only run if motor is actually running (not stopped, not shutdown, not just ramping up)
  const stateSnap = await fetch('/api/motor/state').then(r=>r.json()).catch(()=>null);
  if (stateSnap && !stateSnap.running && !stateSnap.ramping_up) {
    if (!silent) document.getElementById('diagnosis-content').innerHTML='<div class="placeholder">Start motor and run diagnosis to see AI analysis</div>';
    return;
  }
  diagnosisRunning=true;
  const btn=document.getElementById('run-btn');
  if(!silent){btn.disabled=true;btn.textContent='⟳ AI Analyzing...';}
  if(!silent) document.getElementById('diagnosis-content').innerHTML='<div class="diag-loading"><div class="diag-spinner"></div>Gemini AI is analyzing motor data...</div>';
  const payload={
    load_pct:parseFloat(document.getElementById('load').value||70),
    voltage_v:parseFloat(document.getElementById('voltage').value||400),
    rpm_setpoint:parseFloat(document.getElementById('rpm').value||1450),
  ...
  try {
    ...
    const resp=await fetch('/api/diagnose',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),signal:controller.signal});
    clearTimeout(timeout);
    const result=await resp.json();
    renderDiagnosis(result); refreshLog();
  } catch(e) {
    const msg=e.name==='AbortError'?'AI timeout — sensors still live.':'Error: '+e.message;
    document.getElementById('diagnosis-content').innerHTML=`<div style="color:#dc2626;padding:10px;font-size:0.82rem">⚠ ${msg}</div>`;
  } finally {
...
function renderDiagnosis(result) {
  lastResult=result;
  if(result.sensors){history.push(result.sensors);if(history.length>60)history.shift();}
  ...
  fc.innerHTML=flist.length>0?'<div class="fault-tags">'+flist.map(f=>'<span class="fault-tag">'+f+'</span>').join('')+'</div>':'<span class="no-fault">✓ All parameters within normal range</span>';
  updateTrendBars(); checkHistoryAnomalies();
  ...
  if(d&&!d.error){
    ...
    html=`<div style="color:#dc2626;font-size:0.8rem;padding:8px">⚠ ${d.message}</div>`;
  }
...
function resetCountdown() {
  ...
  countdownTimer=setInterval(()=>{
    if(!motorRunning){countdown=30;document.getElementById('countdown').textContent=countdown;return;}
    countdown--;
...
function formatAgentMsg(text) {
  ...
  lines.forEach(line=>{
    const s=line.trim();
    if(s.match(/^[A-Z][A-Z\s]{2,}:/)){const[label,...rest]=s.split(':');html+=`<div class="msg-section">${label}</div>`;if(rest.join(':').trim())html+=`<div class="msg-line">${rest.join(':').trim()}</div>`;}
    else if(s.startsWith('Action ')||s.startsWith('•')||s.startsWith('-')) html+=`<div class="msg-bullet">${s.replace(/^[-•]\s*/,'')}</div>`;
    else html+=`<div class="msg-line">${s}</div>`;
  });
...
async function sendMessage() {
  ...
  const spinner=document.getElementById('spinner'); if(spinner) spinner.style.display='flex';
  try {
    const controller=new AbortController();
    const timeout=setTimeout(()=>controller.abort(),35000);
    const resp=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,last_result:lastResult,history:history.slice(-10)}),signal:controller.signal});
    clearTimeout(timeout);
    const data=await resp.json();
    appendMsg('agent','MotorMind Agent',data.response||'No response.');
  } catch(e){appendMsg('agent','MotorMind Agent',e.name==='AbortError'?'Timeout — try again.':'Error: '+e.message);}
...
async function refreshLog(){
  try{
    const resp=await fetch('/api/log'); const data=await resp.json();
    document.getElementById('log-count').textContent=`(${data.count} events)`;
    const table=document.getElementById('fault-log-table');
    if(!data.events.length){table.innerHTML='<div class="placeholder" style="padding:16px">No WARNING or CRITICAL events yet</div>';return;}
    ...
    data.events.slice().reverse().forEach(e=>{
      ...
      html+=`<tr style="background:${color};border-bottom:1px solid #f1f5f9;"><td style="padding:4px 6px;font-size:0.68rem;color:#64748b">${e.timestamp.slice(11)}</td><td style="color:${tc};font-weight:700;font-size:0.68rem">${e.severity}</td><td style="font-size:0.68rem">${e.voltage_v}V</td><td style="font-size:0.68rem">${e.current_a}A</td><td style="font-size:0.68rem">${e.temperature_c}°C</td><td style="font-size:0.65rem;color:#64748b;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${e.faults}">${e.faults||'—'}</td></tr>`;
    });
...
async function clearLog(){await fetch('/api/log/clear',{method:'POST'});refreshLog();}

// ─── UIPATH LOG REFRESH ───────────────────────────────────────────────────────
async function refreshUiPathLog(){
  try{
    const resp=await fetch('/api/uipath/log'); const data=await resp.json();
    const container=document.getElementById('uipath-log-list');
    if(!data.messages||data.messages.length===0){container.innerHTML='<div style="font-size:0.72rem;color:#94a3b8;text-align:center;padding:8px">No messages yet</div>';return;}
    container.innerHTML=data.messages.slice().reverse().map(m=>{
      const cls=m.type==='shutdown'?'shutdown':m.type==='warning'?'warning':m.type==='approved'?'approved':'info';
      return `<div class="uientry ${cls}"><div class="uet">${m.timestamp} · UiPath</div><div class="uem">${m.message}</div>${m.comment?`<div style="font-size:0.62rem;color:#64748b;margin-top:1px">💬 ${m.comment}</div>`:''}</div>`;
    }).join('');
    // sync motor state from server
    if(data.motor_running!==undefined && data.shutdown_requested!==undefined){
      const wasRunning=motorRunning;
      motorRunning=data.motor_running;
      if(data.shutdown_requested){ applyMotorState({running:false,shutdown_requested:true,stop_type:'emergency'}); }
    }
  }catch(e){}
}
...
window.addEventListener('load', ()=>{
  // Start polling immediately — sensors only run when motor is on
  ...
  resetCountdown();
  // Sync motor state from server on load
  fetch('/api/motor/state').then(r=>r.json()).then(data=>applyMotorState(data)).catch(()=>{});
});
</script>
</body>
</html>
...
def _motor_state_response():
    """Build the full state dict that JS applyMotorState() expects."""
    now = _time.time()
    s = dict(_motor_state)
    elapsed_ms      = int((now - s["start_time"]) * 1000) if s["start_time"] else 0
    stop_elapsed_ms = int((now - s["stop_time"])  * 1000) if s["stop_time"]  else 0
    ramp_done = elapsed_ms > 4000   # 4s ramp-up complete
    ramp_down_done = stop_elapsed_ms > 3000
    return {
        "running":            s["running"] and ramp_done,
        "ramping_up":         s["running"] and not ramp_done,
        "ramping_down":       not s["running"] and s["stop_type"] == "normal" and not ramp_down_done and s["stop_time"] is not None,
        "shutdown_requested": s["shutdown_requested"],
        "shutdown_reason":    s["shutdown_reason"],
        "stop_type":          s["stop_type"],
        "temp_at_stop":       s["temperature_at_stop"],
...
def api_motor_state():
    with _state_lock:
        return jsonify(_motor_state_response())
...
def api_motor_control():
    ...
    now     = _time.time()
    with _state_lock:
        curr_temp = _latest_state.get("temperature_c", 25.0)
        if action == "start":
            _motor_state["running"]            = True
            _motor_state["shutdown_requested"] = False
            _motor_state["shutdown_reason"]    = ""
            _motor_state["start_time"]         = now
            _motor_state["stop_time"]          = None
            _motor_state["stop_type"]          = None
            msg = "Motor started."; mtype = "info"
        elif action == "stop":
            _motor_state["running"]             = False
            _motor_state["shutdown_requested"]  = False
            _motor_state["start_time"]          = None
            _motor_state["stop_time"]           = now
            _motor_state["stop_type"]           = "normal"
            _motor_state["temperature_at_stop"] = curr_temp
            msg = "Motor stopped normally."; mtype = "info"
        elif action == "shutdown":
            _motor_state["running"]             = False
            _motor_state["shutdown_requested"]  = True
            _motor_state["shutdown_reason"]     = comment or "Emergency shutdown"
            _motor_state["start_time"]          = None
            _motor_state["stop_time"]           = now
            _motor_state["stop_type"]           = "emergency"
            _motor_state["temperature_at_stop"] = curr_temp
            msg = f"🚨 EMERGENCY SHUTDOWN — {_motor_state['shutdown_reason']}"; mtype = "shutdown"
        elif action == "reset":
            # Cancel emergency shutdown — back to STOPPED state
            _motor_state["running"]            = False
            _motor_state["shutdown_requested"] = False
            _motor_state["shutdown_reason"]    = ""
            _motor_state["stop_time"]          = None
            _motor_state["stop_type"]          = None
            msg = "Emergency reset — motor in safe STOPPED state."; mtype = "info"
        else:
            return jsonify({"error": "Unknown action"}), 400
        resp = _motor_state_response()
    if source == "uipath" or action in ("shutdown", "reset"):
...
def api_chat():
    ...
    GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY","Token here")
